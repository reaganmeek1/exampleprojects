...
{
    //go through every vehicle move combo and make sure its legal, and if it is, add to listt:
    Board::MovePairList list_legal;
    Board::MovePair legal_pair;
    VehicleMap::const_iterator it = vehicles_.begin();
    for( ; it != vehicles_.end(); ++it){
        for(int j = 1; j < grid_.size(); j++){
            if(isLegalMove(it->first,j)){
              //find out what movepairlist is
                legal_pair = Board::MovePair(it->first, j);
                list_legal.push_back(legal_pair);
                
            }
             if(isLegalMove(it->first,(-1*j))){
                 legal_pair = Board::MovePair(it->first, (-1*j));
                 list_legal.push_back(legal_pair);
             }
        }
    ...
    //make sure ur not returning the type
    return list_legal;
